    ...
    try {
      ...
      if (authData.user) {
        ...
        if (employeeData.role === 'admin') {
          await supabase.auth.signOut();
          setError("Access Denied: You are an administrator. Please use the Admin Portal at /signin/admin");
          toast({
            title: "Wrong Portal",
            description: "Admins must use /signin/admin",
            variant: "destructive",
            duration: 6000,
          });
          setLoading(false);
      ...
      <Card className="w-full max-w-md shadow-xl">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
            ...
            <Button
              type="submit"
              className="w-full h-10 sm:h-11 gradient-primary text-white text-sm sm:text-base"
              disabled={loading}
            >
              {loading ? (
                <>
